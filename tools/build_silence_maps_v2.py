# tools/build_silence_maps_v2.py
...
DEFAULT_MIN_SILENCE_MS = 1400

DEFAULT_MERGE_GAP_MS = 320

DEFAULT_KEEP_LEAD_MS = 140

DEFAULT_KEEP_TAIL_MS = 180

DEFAULT_MIN_SKIP_MS = 900

DEFAULT_MAX_SKIP_MS = 12000
...
def _http_open(url: str, timeout: int = 60):
    ...
    req = Request(
        ...
        headers={
            ...
            "User-Agent": "silence-map-builder-v2/1.0",
...
def save_index(index_path: Path, index_data: dict) -> None:
    ...
    index_data["version"] = DEFAULT_ALGO_VERSION
...
def percentile_clip_floor(values: np.ndarray, q: float) -> float:
...
def detect_silence_segments_v2(
    ...
    rms_values = np.zeros(total_frames, dtype=np.float32)
    ...
    for i in range(total_frames):
        ...
        rms_values[i] = rms_of_frame(frame)

        frame_bounds.append((start_idx / float(sample_rate), end_idx / float(sample_rate)))

    smooth_window = max(3, int(round(240 / max(1, frame_ms))))

    smooth_rms = moving_average(rms_values, smooth_window)

    noise_floor = percentile_clip_floor(smooth_rms, 20)

    adaptive_threshold = max(threshold, noise_floor * 1.35)

    silent_ranges: List[Tuple[float, float]] = []

    current_start: Optional[float] = None

    for i, rms_val in enumerate(smooth_rms):

        t0, t1 = frame_bounds[i]

        silent = rms_val < adaptive_threshold
...
def process_one_item(
    ...
    with tempfile.TemporaryDirectory(prefix="silence-map-v2-") as tmp_dir_raw:
        ...
        segments = detect_silence_segments_v2(
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Silence map v2 batch builder")

    parser.add_argument("--source", choices=sorted(SOURCE_CONFIG.keys()), required=True)

    parser.add_argument("--output", default="silence-maps-v2")
